...
----------------------------
display1 =
gdisplay(x=400,y=0,width=700,height=450,xtitle="Angle",ytitle="Years")
K = gcurve(gdisplay=display1,color=color.blue)
#---------------------------------------------------------------------
-------------------------------------------
while Theta < pi/120:
Moon.rpos = vector(384400*cos(Theta),0,-384400*sin(Theta))
#updates real position of the moon
Moon.opp = vector( -Moon.rpos.x, 0, -Moon.rpos.z) #opposite
position of moon, used for gravitational field
Moon.pos =(50000*cos(Theta),0,-50000*sin(Theta)) #updates
simulated position of the moon
Earth.rotate(angle=dPhi,axis=(0,1,0)) #rotates the earth by dPhi
Ocean_Frame.rotate(angle=dPhi,axis=(0,1,0)) #rotates the ocean
frame by dPhi

 
for object in Ocean: #determines Fg for each water molecule and
...
 
if object.rpos.z < 0:
object.beta = diff_angle(vector(1,0,0),vector(object.rpos.x,
0,object.rpos.z))
if object.rpos.z <= 0:
object.beta = -diff_angle(vector(1,0,0),vector(object.rpos.x,
0,object.rpos.z))
 
if object.beta > 0 :
High_Tide_Grav = High_Tide_Grav + GMm/mag2(Moon.rpos-
object.rpos)
if object.beta < 0 and object.beta > -pi/2:
Low_Tide_Grav = Low_Tide_Grav + GMm/mag2(Moon.rpos-
...
(50+i)),0,(Earth_Radius-Ocean_Depth/2)*sin(pi/(50+i)))
Quad_1_Pull = G*Quad_1_Tide_Mass*Moon_Mass/mag2(Moon.pos-
High_Tide_Point)*norm(vector(384400,0,0)-High_Tide_Point)
Quad_4_Pull = G*Quad_4_Tide_Mass*Moon_Mass/mag2(Moon.pos-
Low_Tide_Point)*norm(vector(384400,0,0)-Low_Tide_Point)
